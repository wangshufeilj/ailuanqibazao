...
本仓库**不存储**图片、视频、PDF、音频等大文件，在每个大文件的**保存目录**下有一份同名 `.md` 元信息文件。表格（xlsx、csv 等）可直接提交。详见 [LARGE_FILES_MANIFEST.md](LARGE_FILES_MANIFEST.md)。实际文件需在本地保有。
...
4. 更新 `INDEX.md`；大文件须在其同目录下创建同名 `.md` 元信息文件
5. 空目录放入 `.gitkeep` 以便 Git 保留目录结构
